...
public class NGramMap {
    ...
    public IPriorityQueue.PQElement<String>[] getCountsAfter(NGram ngram) {
        ...
        IDictionary<IterableString, Integer> words = this.map.get(ngram);
        IPriorityQueue.PQElement<String>[] result = new IPriorityQueue.PQElement[words.size()];
        int c = 0;
        for (IterableString word : words) {
            int count = this.map.get(ngram).get(word);
            IPriorityQueue.PQElement<String> elem = new IPriorityQueue.PQElement<>(word.toString(), count);
            result[c] = elem;
            c++;
        }
        return result;
    ...
    public String[] getWordsAfter(NGram ngram, int k) {
        ...
        // sort array of <words, count>
        IPriorityQueue.PQElement<String>[] array = getCountsAfter(ngram);
        if(array == null){
            return new String[0];
        }
        int min = Math.min(array.length, k);
        String[] result = new String[min];
        TopKSort.sort(array, k);

        // get the words in array and put them into result
        for (int i = 0; i < min; i++) {
            result[i] = array[i].data;
        }
